...
interface InfiniteGalleryProps {
	...
	/** Called once when all images have been shown (one full cycle) */
	onComplete?: () => void;
	/** Called when all images have finished loading initially */
	onLoaded?: () => void;
}
...
function GalleryScene({
	...
	// Notify parent when textures are loaded
	useEffect(() => {
		onLoaded?.();
	}, [onLoaded]);
	...
	useFrame((state, delta) => {
		// Apply auto-play
		if (autoPlay && !completed.current) {
			setScrollVelocity((prev) => prev + 0.3 * delta);
		}

		// Track total scrolled distance to detect completion
		if (!completed.current) {
			const frameDist = Math.abs(scrollVelocity * delta * 10);
			totalScrolled.current += frameDist;
			if (totalScrolled.current >= DEFAULT_DEPTH_RANGE) {
